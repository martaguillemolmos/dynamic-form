...
const submitStatus = ref(null) // Para manejar estado de envío
...
onMounted(async () => {
  try {
    ...
    formData.value = form.value.fields.reduce((acc, field) => {
      acc[field.name] = field.defaultValue || '' // Usa defaultValue si existe
      return acc
    }, {})
...
const handleSubmit = async () => {
  try {
    ...
    const response = await axios.post(`/api/form-templates/${formId.value}/responses`,
      formData.value,
      {
        headers: {
    ...
    // Reseteamos de nuevo el formulario
